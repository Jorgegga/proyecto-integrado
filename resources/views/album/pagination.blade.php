            ...
            <a href="{{route('verAlbum', ['album' => $item, 'nombre'=> $item->nombre])}}">
                <div class="card col-md-3 col-sm-3 mr-sm-5 mr-md-5 pt-3 mt-4"
                    ...
                    <div class="card-footer text-center mt-auto" style="background-color:#212E36;">
                        <p><a href="{{route('verAlbum', ['album' => $item, 'nombre'=> $item->nombre])}}" class="btn btn-primary mb-1">Ir al álbum</a></p>
                        <p><button class="btn btn-success" data-toggle="modal" data-target="#albumRaw" role="tab" title="Raw"
                            onclick="carga('{{ $item->id }}', '{{$item->nombre}}')">Modo ventana</button></p>
                            <a href="{{route('verAutor', ['autor' => $item->autor->id, 'nombre'=> $item->autor->nombre])}}" class="text-muted text-left" style="float:left;">
                                {{ $item->autor->nombre }}</a>
